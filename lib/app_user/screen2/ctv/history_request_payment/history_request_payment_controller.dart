...
class HistoryRequestPaymentController extends GetxController {
  ...
  Future<void> getHistoryPayment() async {
    try {
      var data = await RepositoryManager.ctvRepository.getHistoryPayment(sortBy: "id");
      listInit = data!.data!;
  ...
  void findRequestPayment({bool? isClose}) {
    if (isClose == true) {
      ...
      if (inputTextEditingController.text.isNum) {
        ...
        print(TiengViet.parse(inputTextEditingController.text));
        List<RequestPayment> output = listRequestPaymentShow
            .where((itemRequest) =>
            
            TiengViet.parse('${itemRequest.ctv?.customer?.name}')
                .toLowerCase()
