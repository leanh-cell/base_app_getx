...
class SaleMarketController extends GetxController {
  ...
  double? longitude;
  String? deviceName;
  var loadCheckIn = false.obs;
  int? indexCheckin;
  SaleMarketController() {
    getListAgencyForSale(isRefresh: true);
    initFind();
    getDeviceInfo();
  }

  Future<void> getListAgencyForSale({
    bool? isRefresh,
  }) async {
    if (isRefresh == true) {
      currentPage = 1;
    ...
    try {
      if (isEnd == false) {
        ...
        var data =
            await RepositoryManager.agencyRepository.getListAgencyForSale(
          page: currentPage,
        );

        isHaveCheckOut = data?.isStaffHaveCheckoutAgency;

        if (isRefresh == true) {
          listAgency(data!.data!.data!);

          listAgency.refresh();
        } else {
          listAgency.addAll(data!.data!.data!);
        }

        if (data.agencyAreCheckin != null) {
          listAgency.removeWhere((e) => e.id == data.agencyAreCheckin?.id);
          listAgency.insert(0, data.agencyAreCheckin!);
        }
  ...
  Future<void> GetAddressFromLatLong(Position position) async {
    try {
      ...
      if (placeMarks.isNotEmpty) {
        ...
        var addr =
            '${place.name}, ${place.street}, ${place.subLocality}, ${place.subAdministrativeArea}, ${place.locality}, ${place.country}';
        addressCheckin = addr;
        print("===========================> $addr");
      }
    } catch (err) {
  ...
  Future<void> checkInAgency({required SaleCheckIn saleCheckIn}) async {
    try {
      var res = await RepositoryManager.agencyRepository
          .checkInAgency(saleCheckIn: saleCheckIn);
    } catch (e) {
      SahaAlert.showError(message: e.toString());
    }
  }

  Duration? countTimeVisit({required DateTime timeCheckIn}) {
    var timeVisit = DateTime.now().difference(timeCheckIn);
    return timeVisit;
  }
  ...
  void getDeviceInfo() async {
    ...
    if (Platform.isAndroid) {
      ...
      print('Running on ${iosInfo.name}  ${iosInfo.model} ${appleDevice[iosInfo.utsname.machine] ?? 'Apple Device'}');
      deviceName = appleDevice[iosInfo.utsname.machine] ?? 'Apple Device';
    }
  }

final Map<String, String> appleDevice = {
  'iPhone1,1': 'iPhone',
  'iPhone1,2': 'iPhone 3G',
