import 'package:com.ikitech.store/app_user/data/remote/response-request/agency/all_agency_register_request_res.dart';
import 'package:com.ikitech.store/app_user/model/agency_type.dart';
import 'package:get/get.dart';

import '../../../components/saha_user/toast/saha_alert.dart';
import '../../../data/remote/response-request/ctv/all_collaborator_register_request_res.dart';
import '../../../data/repository/repository_manager.dart';

class AgencyRequestRegisterController extends GetxController{
  var listRegisterRequest = RxList<AgencyRegisterRequest>();
  int currentPage = 1;
  int status = 0;
  bool isEnd = false;
  var isLoading = false.obs;
  var loadInit = true.obs;
  var listAgencyType = RxList<AgencyType>();

  AgencyRequestRegisterController() {
    getAllAgencyRegisterRequest(isRefresh: true);
    getAllAgencyType();
  }

  Future<void> getAllAgencyRegisterRequest({
    bool? isRefresh,
  }) async {
    if (isRefresh == true) {
      currentPage = 1;
      isEnd = false;
    }

    try {
      if (isEnd == false) {
        isLoading.value = true;
        var data = await RepositoryManager.agencyRepository
            .getAllAgencyRegisterRequest(
          page: currentPage,
          status: status,
        );

        if (isRefresh == true) {
          listRegisterRequest(data!.data!.data!);
          listRegisterRequest.refresh();
        } else {
          listRegisterRequest.addAll(data!.data!.data!);
        }

        if (data.data!.nextPageUrl == null) {
          isEnd = true;
        } else {
          isEnd = false;
          currentPage = currentPage + 1;
        }
      }
      isLoading.value = false;
      loadInit.value = false;
    } catch (err) {
      SahaAlert.showError(message: err.toString());
    }
  }

  Future<void> updateStatusCollaboratorRequest(
      {required int requestId, required int status,int? agencyTypeId}) async {
    try {
      var res = await RepositoryManager.agencyRepository
          .updateAgencyRegisterStatus(
              idRequest: requestId, status: status,agencyTypeId: agencyTypeId);
     SahaAlert.showSuccess(message: "Thành công");
    } catch (e) {
      SahaAlert.showError(message: e.toString());
    }
  }

   Future<void> getAllAgencyType() async {
    try {
      var data = await RepositoryManager.agencyRepository.getAllAgencyType();
      listAgencyType(data!.data!);
    } catch (err) {
      SahaAlert.showError(message: err.toString());
    }
  }
}